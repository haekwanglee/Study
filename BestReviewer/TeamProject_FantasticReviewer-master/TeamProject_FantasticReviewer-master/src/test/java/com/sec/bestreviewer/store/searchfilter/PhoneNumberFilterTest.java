...
class PhoneNumberFilterTest {

    @Test
    @DisplayName("동일 필드 값을 가진 Employee 검색")
    void searchByPhoneNumberReturnExpected() {
        ...
        // invoke
        List<Employee> ret = new PhoneNumberFilter().search(employees, param);
    ...
    @ParameterizedTest
    @DisplayName("전화번호 부등호를 활용한 검색 테스트")
    @CsvSource({
    ...
    void searchPhoneNumberBySignReturnExpected(String signTag, int expectedSize, String message) {

        // prepare
        final List<Employee> employees = new CommandGenerator().generateEmployees(10).stream()
                .sorted(Comparator.comparing(Employee::getPhoneNumber))
