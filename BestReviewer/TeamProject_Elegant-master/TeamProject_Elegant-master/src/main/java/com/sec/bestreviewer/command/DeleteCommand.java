...
public class DeleteCommand extends CommandImpl {
    public DeleteCommand(CommandDTO commandDTO) {
        super(commandDTO);
        this.setModelParams();
    }
    ...
    public List<String> execute(EmployeeStore employeeStore) {
        final List<Employee> employeeList = employeeStore.delete(modelParams);

        if (commandDTO.isPrintOptionOn()) {
            return ResultStringFormatter.getEmployeeListToFormattedString(
                    employeeList, CommandType.DEL, MAX_RESULT_NUMBER);
        }
        return ResultStringFormatter.getEmployeeListToFormattedString(employeeList, CommandType.DEL);
    }
